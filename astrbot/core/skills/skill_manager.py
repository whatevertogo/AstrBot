...
class SkillManager:
    ...
    def materialize_local_skill_bundle(
        ...
        *,
        ...
        for skill_name, plugin_id, skill_dir in self._iter_plugin_skill_dirs():
            if selected_names is not None and skill_name not in selected_names:
                continue
            if any(source.name == skill_name for source in copied_sources):
                continue
            skill_md_path = _normalize_skill_markdown_path(
                skill_dir,
                rename_legacy=False,
            )
            if skill_md_path is None:
